  ...
  persist(
    (set, get) => ({
      ...
      setMode: (mode: TimerMode) => {
        ...
        // On ne réinitialise le temps que si le timer est à son état initial (non démarré et temps par défaut)
        const isAtDefault = currentState.timeLeft === MODE_DURATIONS[currentState.mode] && !currentState.isRunning;
        
        const newTimeLeft = isAtDefault ? MODE_DURATIONS[mode] : currentState.timeLeft;
        ...
        syncChannel?.postMessage({ 
          type: 'SYNC', 
          payload: { mode, timeLeft: newTimeLeft, isRunning: currentState.isRunning, expectedEndTime: currentState.expectedEndTime } 
        });
      },
